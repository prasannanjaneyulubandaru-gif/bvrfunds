...
// ===========================================
// DEPLOYMENT RESULTS RENDERING
// ===========================================

/**
 * Render deployment results with proper status display
 * @param {Object} summary - Deployment summary from backend
 * @param {string} containerId - ID of container element to render results in
 */
function renderDeploymentResults(summary, containerId) {
    ...
    const { total, successful, failed, results } = summary;
    
    let html = `
        <div class="deployment-summary mb-4">
            <h3 class="text-xl font-bold mb-3">Deployment Complete</h3>
        ...
        <div class="deployment-results space-y-2">
    `;
    ...
    results.forEach(result => {
        ...
        html += `
            <div class="${bgColor} border-2 ${statusClass.split(' ')[2]} rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        <div class="font-bold text-lg">${result.symbol}</div>
                        <div class="text-sm text-gray-600">
                            Order ID: ${result.order_id || 'N/A'} • Lots: ${result.lots || 0} • Qty: ${result.quantity || 0}
                        </div>
                        ${result.status_message ? `
                            <div class="mt-2 text-sm ${result.success ? 'text-green-700' : 'text-red-700'} font-medium">
                                ${result.status_message}
                            </div>
                        ` : ''}
                    </div>
                    <div class="ml-4">
                        <span class="px-3 py-1 ${statusClass} text-sm font-semibold rounded-full border">
                            ${statusIcon} ${status}
                        </span>
                    </div>
                </div>
...
window.BasketManager = window.BasketManager || {};
window.BasketManager.renderDeploymentResults = renderDeploymentResults;
