...
function showTrailSlConfig() {
    ...
    contentDiv.innerHTML = `
        ...
        <div class="p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
            ...
            <p class="mt-2">
                The system will place a <strong>SL (Stop Loss Limit)</strong> order ${isLong ? 'below' : 'above'} your average price with a wide 5% limit buffer. This ensures the order executes reliably for F&O contracts while providing price protection.
            </p>
        </div>
    `;
            ...
            messagesDiv.innerHTML = `
                <div class="p-4 bg-green-50 border-2 border-green-200 rounded-lg">
                    ...
                    <div class="mt-2 text-xs text-gray-600">
                        💡 Using SL limit order with wide buffer for F&O compatibility
                    </div>
                </div>
            `;
        ...
        if (data.success) {
            ...
            messagesDiv.innerHTML = `
                <div class="p-3 bg-green-50 border-2 border-green-200 rounded-lg text-sm">
                    ✅ Trigger adjusted to ₹${currentTrigger.toFixed(2)} (Limit: ₹${limitPrice.toFixed(2)})
                </div>
            `;
        } else {
